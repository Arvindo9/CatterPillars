...
import com.solution.catterpillars.databinding.MemberListAdapterBinding;
 ...
 */
public class MemberAdapter extends BaseAdapter<MemberListAdapterBinding, List> {

    public MemberAdapter(Context context, MemberFragment memberFragment,
                         ArrayList<List> list,
                         MemberViewModel viewModel) {
        super(context, list);
    }

    protected void hideView(MemberListAdapterBinding binding) {

    }

    @Override
    protected void showView(MemberListAdapterBinding binding) {
    ...
    protected int getLayout() {
        return R.layout.member_list_adapter;
    }
     ...
     */
    @Override
    public ViewHolder getViewHolder(MemberListAdapterBinding binding) {
        return new ViewHolder(binding) {
            /**
             ...
             */
            @Override
            protected void doSomeWorkHere(MemberListAdapterBinding binding, List data, int position) {
            }
            ...
            public void setClickListeners(ViewHolderClickListener thisContext,
                                          MemberListAdapterBinding binding, List data) {
             ...
             */
            @Override
            protected ViewHolderClickListener viewHolderReference(MemberListAdapterBinding binding,
    ...
    protected FilterClass initialisedFilterClass() {
        return new FilterClass() {
            ...
            public ArrayList<List> filterData(CharSequence constraint, ArrayList<List> list,
                ...
                for (List data : list) {
                    if (data.getMemberName().toLowerCase().contains(constraint) ||
                            data.getMobile().toLowerCase().contains(constraint)||
                            data.getEmail().toLowerCase().contains(constraint)||
                            data.getSignupTaskStatus().toLowerCase().contains(constraint)||
                            data.getDailyTaskStatus().toLowerCase().contains(constraint)
                            )
                        filteredArrayList.add(data);
                }
